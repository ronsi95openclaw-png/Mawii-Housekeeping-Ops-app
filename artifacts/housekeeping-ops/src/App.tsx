...
function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? 'sidebar-open' : ''}`}>
      <div className="brand"><span className="brand-mark"><img src="/mawii-logo.jpeg" alt="" /></span><span><b>Mawii Property Care</b><small>operations desk</small></span><button className="close-sidebar" onClick={() => setOpen(false)} data-testid="button-close-menu"><X size={17} /></button></div>
      ...
      <div className="sidebar-bottom">
        <div className="sync-line"><span className={`status-dot ${health.isError ? 'status-dot-warn' : ''}`} />{health.isLoading ? 'Checking connection' : health.isError ? 'Connection delayed' : 'Mawii operations online'}</div>
        <div className="user-card"><Avatar member={{ id: 0, name: 'Danna Donjuan', role: 'Owner', phone: '', status: 'available' }} /><div><strong>Danna Donjuan</strong><small>Owner · Operations desk</small></div><button className="icon-button" data-testid="button-user-menu"><ChevronRight size={15} /></button></div>
      </div>
    </aside>
...
function Jobs() {
  ...
  if (jobs.isError) return <ErrorState onRetry={() => void jobs.refetch()} />;
  const submitCreate = (data: NewJobForm) => create.mutate({ data }, { onSuccess: () => { setShowCreate(false); void queryClient.invalidateQueries({ queryKey: getListJobsQueryKey() }); } });
  return <div className="content-stack">
...
function CreateJobDialog({ onClose, onSubmit, pending }: { onClose: () => void; onSubmit: (data: NewJobForm) => void; pending: boolean }) {
  ...
  const [form, setForm] = useState({ clientName: '', address: '', scheduledDate: todayISO(), startTime: '09:00', endTime: '12:00', serviceType: 'Standard cleaning', serviceVariant: '2 bed / 2 bath Standard', addOns: [] as string[], durationMinutes: 180, frequency: 'Every 4 weeks', notes: '', clientPhone: '', assignedMemberId: '' });
  ...
  return <div className="modal-scrim"><form className="modal panel" onSubmit={(e: FormEvent) => { e.preventDefault(); const { assignedMemberId, ...job } = form; onSubmit({ ...job, teamMemberIds: assignedMemberId ? [Number(assignedMemberId)] : [] }); }}><div className="modal-head"><div><span className="eyebrow">Elevate OS intake</span><h3>Add a job</h3></div><button type="button" className="icon-button" onClick={onClose} data-testid="button-close-create-job"><X size={17} /></button></div><div className="form-grid"><label>Client name<input required value={form.clientName} onChange={(e) => update('clientName', e.target.value)} data-testid="input-job-client" /></label><label>Client phone<input value={form.clientPhone} onChange={(e) => update('clientPhone', e.target.value)} data-testid="input-job-phone" /></label><label className="span-2">Service address<input required value={form.address} onChange={(e) => update('address', e.target.value)} data-testid="input-job-address" /></label><label>Date<input type="date" required value={form.scheduledDate} onChange={(e) => update('scheduledDate', e.target.value)} data-testid="input-job-date" /></label><label>Assigned worker<select value={form.assignedMemberId} onChange={(e) => update('assignedMemberId', e.target.value)} data-testid="input-job-worker"><option value="">Unassigned</option>{(team.data || []).map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><label>Start<input type="time" required value={form.startTime} onChange={(e) => update('startTime', e.target.value)} data-testid="input-job-start" /></label><label>End<input type="time" required value={form.endTime} onChange={(e) => update('endTime', e.target.value)} data-testid="input-job-end" /></label><label>Appointment<select required value={form.serviceType} onChange={(e) => update('serviceType', e.target.value)} data-testid="input-job-service">{SERVICE_OPTIONS.map((service) => <option key={service}>{service}</option>)}</select></label><label>Variant<input value={form.serviceVariant} onChange={(e) => update('serviceVariant', e.target.value)} data-testid="input-job-variant" /></label><label>Duration (minutes)<input type="number" min="0" value={form.durationMinutes} onChange={(e) => setForm({ ...form, durationMinutes: Number(e.target.value) })} data-testid="input-job-duration" /></label><label>Frequency<input value={form.frequency} onChange={(e) => update('frequency', e.target.value)} data-testid="input-job-frequency" /></label><fieldset className="span-2 add-on-field"><legend>Add-ons</legend><div>{['Laundry', 'Inside Oven', 'Inside Fridge', 'Inside Cabinets'].map((addOn) => <label key={addOn}><input type="checkbox" checked={form.addOns.includes(addOn)} onChange={() => toggleAddOn(addOn)} />{addOn}</label>)}</div></fieldset><label className="span-2">Additional information<textarea rows={3} value={form.notes} onChange={(e) => update('notes', e.target.value)} placeholder="Access instructions or client requests" data-testid="input-job-notes" /></label></div><div className="modal-actions"><button type="button" className="button button-secondary" onClick={onClose} data-testid="button-cancel-create-job">Cancel</button><button className="button button-primary" disabled={pending} data-testid="button-submit-create-job">{pending ? 'Creating…' : 'Create job'}<ArrowRight size={15} /></button></div></form></div>;
}
...
function Settings() {
  ...
  return <div className="content-stack"><PageIntro eyebrow="Control room" title="Settings" body="Set the defaults that keep a small team in sync." /><div className="settings-layout"><section className="panel settings-card"><div className="section-heading"><div><span className="eyebrow">Notifications</span><h3>Keep the right people posted</h3></div><Bell size={18} className="muted-icon" /></div><div className="setting-list">{[{ key: 'morning', title: 'Morning dispatch', body: 'A 7:00 AM recap of today’s jobs and assignments.' }, { key: 'attention', title: 'Attention alerts', body: 'Tell owners when a job needs a decision or is running late.' }, { key: 'proof', title: 'Proof trail updates', body: 'Notify the desk when photos or checklists are added.' }, { key: 'weekly', title: 'Weekly wrap', body: 'A Friday summary of completed jobs and open follow-ups.' }].map((item) => <div className="setting-row" key={item.key}><div><strong>{item.title}</strong><span>{item.body}</span></div><button className={`toggle ${prefs[item.key as keyof typeof prefs] ? 'toggle-on' : ''}`} onClick={() => toggle(item.key as keyof typeof prefs)} aria-pressed={prefs[item.key as keyof typeof prefs]} data-testid={`button-toggle-${item.key}`}><i /></button></div>)}</div></section><section className="panel settings-card"><div className="section-heading"><div><span className="eyebrow">Company profile</span><h3>How your team shows up</h3></div><Home size={18} className="muted-icon" /></div><div className="company-lockup"><img src="/mawii-logo.jpeg" alt="Mawii Property Care logo" /><div><strong>{company.name}</strong><span>Clean spaces. Better places.</span></div></div><div className="form-stack"><label>Company name<input value={company.name} onChange={(e) => setCompany({ ...company, name: e.target.value })} data-testid="input-company-name" /></label><label>Primary dispatch phone<input value={company.phone} onChange={(e) => setCompany({ ...company, phone: e.target.value })} data-testid="input-company-phone" /></label><label>Service area<input value={company.serviceArea} onChange={(e) => setCompany({ ...company, serviceArea: e.target.value })} data-testid="input-company-area" /></label></div><button className="button button-primary save-button" onClick={saveCompany} data-testid="button-save-settings">{saved ? <><Check size={15} />Saved</> : 'Save company settings'}</button></section></div><section className="panel service-catalog"><div className="section-heading"><div><span className="eyebrow">Service structure</span><h3>Mawii pricing and crew standards</h3></div><Badge tone="blue">$35 / hour</Badge></div><div className="service-grid">{services.map((service) => <article key={service.name}><strong>{service.name}</strong>{service.rows.map((row) => <span key={row}>{row}</span>)}<small>Each additional room adds 30 minutes.</small></article>)}</div><div className="service-notes"><div><strong>Add-ons · +30 minutes each</strong><span>Laundry · Inside Oven · Inside Fridge · Inside Cabinets</span></div><div><strong>Contractor standards</strong><span>Bring cleaning supplies and a vacuum. Wear all-black attire or solid scrubs. Maintain a professional and respectful presence.</span></div></div></section></div>;
}
