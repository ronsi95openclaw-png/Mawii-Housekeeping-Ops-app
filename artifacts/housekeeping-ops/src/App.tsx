...
function JobDetail({ job }: { job: Job }) {
  ...
  return <section className="panel job-detail" data-testid={`job-detail-${job.id}`}><div className="detail-top"><div><span className="eyebrow">Job #{String(current.id).padStart(4, '0')} · {formatDate(current.scheduledDate, { weekday: 'long', month: 'short', day: 'numeric' })}</span><h2>{current.clientName}</h2><p><MapPin size={14} />{current.address}</p></div><select value={current.status} onChange={(e) => patch({ status: e.target.value as 'scheduled' | 'in_progress' | 'completed' | 'attention' })} data-testid="select-job-status"><option value="scheduled">Scheduled</option><option value="in_progress">In progress</option><option value="attention">Attention</option><option value="completed">Completed</option></select></div><div className="detail-stat-row"><div><span>Window</span><strong>{formatTime(current.startTime)} – {formatTime(current.endTime)}</strong></div><div><span>Service</span><strong>{current.serviceType}</strong></div><div><span>Contact</span><strong>{current.clientPhone || 'Not provided'}</strong></div></div><div className="detail-section"><div className="detail-section-head"><div><span className="eyebrow">Closeout</span><h3>Checklist</h3></div><span className="mono">{current.checklist?.filter((i) => i.completed).length || 0}/{current.checklist?.length || 0}</span></div>{current.checklist?.length ? <div className="checklist">{current.checklist.map((item) => <label className={`check-row ${item.completed ? 'check-complete' : ''}`} key={item.id}><input type="checkbox" checked={item.completed} onChange={(e) => toggleChecklist(item.id, e.target.checked)} data-testid={`checkbox-checklist-${item.id}`} /><span>{item.label}</span>{item.completed && <Check size={15} />}</label>)}</div> : <p className="muted-copy">No checklist items have been added to this job.</p>}</div><div className="detail-section"><div className="detail-section-head"><div><span className="eyebrow">Crew</span><h3>Assigned team</h3></div><button className="text-button" onClick={() => patch({ teamMemberIds: [] })} data-testid="button-clear-team">Clear team</button></div><div className="assigned-team">{current.team?.length ? current.team.map((member) => <div className="assigned-member" key={member.id}><Avatar member={member} /><div><strong>{member.name}</strong><span>{member.role}</span></div><a href={`tel:${member.phone}`} className="icon-button" data-testid={`link-call-team-${member.id}`}><Phone size={14} /></a></div>) : <span className="muted-copy">No team assigned.</span>}</div></div><div className="detail-section proof-section"><div className="detail-section-head"><div><span className="eyebrow">Proof trail</span><h3>Photos</h3></div><Badge tone={current.photos?.length ? 'green' : 'neutral'}>{current.photos?.length || 0} uploaded</Badge></div>{current.photos?.length ? <div className="photo-grid">{current.photos.map((photo) => <img key={photo.id} src={photo.url} alt={photo.label} data-testid={`img-proof-${photo.id}`} />)}</div> : <div className="proof-empty"><CheckCircle2 size={17} /><span>Photos from the crew will land here when the job is closed.</span></div>}</div><div className="message-box"><div><span className="eyebrow">Client nudge</span><h3>Send a message</h3></div><div className="message-input"><input value={message} onChange={(e) => setMessage(e.target.value)} placeholder="e.g. We’ll see you between 9 and 10." data-testid="input-job-message" /><button disabled={!message.trim() || sendMessage.isPending} onClick={() => sendMessage.mutate({ id: job.id, data: { recipient: 'client', body: message.trim() } }, { onSuccess: () => setMessage('') })} data-testid="button-send-job-message"><Send size={15} /></button></div><span className="message-note"><MessageSquare size={13} /> Goes to {current.clientName} · {sendMessage.isSuccess ? 'Sent just now' : 'Messages are logged to this job'}</span></div></section>;
}
