...
router.post("/jobs/:jobId/complete", async (req, res) => {
  ...
  if (checklist.some(item => !item.completed)) { res.status(409).json({ error: "Checklist must be complete" }); return; }
  if (!photos.some(photo => photo.kind === "before") || !photos.some(photo => photo.kind === "after")) { res.status(409).json({ error: "Before and after photos are required" }); return; }
  const employee = await currentEmployee(req);
